...
while End == False:
    ...
    try:
        ...
        print("You entered text. Please input a valid number.")  #If the diameter cannot be turned into an integer, it means the input was text. A ValueError will be raised when it is not able to be turned into an integer. The program will recognize this error and know to print the message.
    else:
        if float(diameter) < 0:        #If the diameter is able to become an integer, the code will continue down to here, where the program checks if the diameter is a negative number. If it is, it will print the message.
            print("You entered a negative number. Please input a valid number.")
        else:
